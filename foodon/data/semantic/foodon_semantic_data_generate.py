...
ONTOLOGY_PATH = "foodon.owl"
CLASS_FILE = Path.cwd().parent / "owl2vec" / "classes.txt"
OUTPUT_PATH = "food_on_filtered_subsumption_dataset.pkl"
...
foodon_cls: list = list(filter(lambda x: x.__name__[:6] == "FOODON", all_classes))
print(f"{len(foodon_cls) = }")

dataset = []
for cls in filter(is_most_specific, foodon_cls):
